...
describe('şekil betimleyicileri', () => {
  it('dolu daireyi dairesel ve delik­siz ölçer', async () => {
    const px = tuval()
    daire(px, C, C, 55)
    const { lekeler } = await fincaniAnalizEt(await pngYap(px))
  ...
  it('çizgiyi uzun ve yönlü ölçer', async () => {
    ...
    const dikey = await fincaniAnalizEt(await pngYap(dikeyPx))
    ...
    const yatay = await fincaniAnalizEt(await pngYap(yatayPx))
...
describe('bölge ve konum', () => {
  it('lekeyi bulunduğu bölgeye yerleştirir', async () => {
    ...
    const dip = await fincaniAnalizEt(await pngYap(dipPx))
  ...
  it('sağa yığılmış telveyi sağ yarıda daha yoğun ölçer', async () => {
    ...
    const a = await fincaniAnalizEt(await pngYap(px))
...
describe('okuma bütünlüğü', () => {
  it('aynı fotoğraf her seferinde aynı falı verir', async () => {
    ...
    const bir = faliYorumla(await fincaniAnalizEt(png))
    const iki = faliYorumla(await fincaniAnalizEt(png))
  ...
  it('farklı fotoğraflar farklı fal üretir', async () => {
    ...
    const falA = faliYorumla(await fincaniAnalizEt(await pngYap(a)))
    const falB = faliYorumla(await fincaniAnalizEt(await pngYap(b)))
  ...
  it('boş fincanı açık ve ferah okur', async () => {
    const analiz = await fincaniAnalizEt(await pngYap(tuval()))
  ...
  it('yoğun fincanda birden fazla sembol okur', async () => {
    ...
    const fal = faliYorumla(await fincaniAnalizEt(await pngYap(px)))
